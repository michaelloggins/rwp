...
GO
-- =============================================
-- Author:		<Author,,O'Connell,Sean>
-- Create date: <Create Date,, 08/1/2022>
-- Description:	<Description,,Get Results in timeframe and add prices to temp table used by excel>
-- =============================================
CREATE PROCEDURE  [dbo].[sp_ResultsWithPricing]
...
BEGIN
	...
	SET NOCOUNT ON;
(Select  r.ANALYTE, CR.BIRTH_DATE, cr.CLIENT_SAMPLE_ID, cr.DATE_COLLECTED, cr.EXTERNAL_ID, cr.FIRST_NAME, cr.LAST_NAME, cr.SPEC_SOURCE,
   rc.CATEGORY, rc.COMPNAME, r.DATEENTER, r.FINAL, r.NUMRES, r.RN2, r.RN3,
   		CASE
			WHEN r.TESTCODE = '318' THEN '316'
			WHEN r.TESTCODE = '457' THEN '331'
...
where (r.DATEENTER BETWEEN  @CalDate AND @CalEnd) and rc.COMPNAME not in ('Test RightFax 1', 'Test RightFax 4', 'ACCL0933', 'RESEARCH ONLY', 'MiraVista Quality System', 'Greater Than', 'Additional Positive Unconfirmed', 'ZZZ%')
...
and rc.COMPNAME like 'Santa Clara%' and r.TESTCODE = 315 and UPPER(p.STATUS) = 'RELEASED'  AND r.TESTCODE NOT IN (432,429,443,440))
	...
	SELECT DISTINCT
    MIN(CR.BIRTH_DATE)           AS BIRTH_DATE,
    MIN(cr.CLIENT_SAMPLE_ID)     AS CLIENT_SAMPLE_ID,
    MIN(cr.DATE_COLLECTED)       AS DATE_COLLECTED,
    MIN(cr.EXTERNAL_ID)          AS EXTERNAL_ID,
    MIN(cr.FIRST_NAME)           AS FIRST_NAME,
    MIN(cr.LAST_NAME)            AS LAST_NAME,
